...
def find_template(explicit):
    """Путь к шаблону index.html. Ищется рядом со скриптом, затем в текущей
    директории; можно задать явно через --template."""
    if explicit:
    ...
    raise ReportError(
        ...
        "Положите index.html рядом со скриптом или укажите его: --template путь/к/index.html"
    )
...
def build_html(template_html, good, broken):
    # вставляем перед ПОСЛЕДНИМ </body> — в index.html токен </body> может
    # встречаться в комментарии внутри скрипта, а настоящий тег — последний.
    idx = template_html.rfind("</body>")
    if idx < 0:
        raise ReportError("Шаблон не похож на index.html: нет </body>.")
    injected = make_data_block(good, broken) + BOOTSTRAP
    return template_html[:idx] + injected + template_html[idx:]
